...
function strengthBarColor(strength: PasswordStrength): string {
...
export default function EntryFormPage() {
  ...
  const handleDelete = useCallback(async () => {
    ...
    const confirmed = window.confirm(
      "¿Eliminar esta contraseña? Esta acción no se puede deshacer."
    );
    if (!confirmed) return;
    setIsDeleting(true);
    setSaveError("");
    try {
      ...
      setSaveError("Error al eliminar. Comprueba tu conexión.");
    } finally {
      setIsDeleting(false);
    }
  }, [id, deleteEntry, save, navigate]);

  const cancelTo = isNew ? "/vault" : `/entry/${id}`;
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-[var(--bg-card)] px-4 md:px-6 py-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h2 className="text-lg font-bold leading-tight tracking-tight text-[var(--text)] truncate">
            {isNew ? "Nueva contraseña" : "Editar contraseña"}
          </h2>
        </div>
          ...
          >
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex w-full items-stretch">
                  ...
                  <button
                    ...
                    className="flex border border-slate-300 dark:border-slate-600 border-l-0 rounded-r-lg bg-white dark:bg-slate-900 items-center justify-center px-3 text-slate-500 dark:text-slate-400 hover:text-primary-500 transition-colors"
                ...
                {strength && (
                  <div className="mt-2 space-y-2">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Fortaleza:{" "}
                      <span className={`font-bold ${strengthTextColor(strength)}`}>
                        {strength.label}
                      </span>
                    </p>
                  </div>
              ...
              <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {[
                    ...
                    {
                      label: "Excluir ambiguos (0O1lI)",
                      value: genExcludeAmbiguous,
                      set: setGenExcludeAmbiguous,
                    },
                    {
                      label: "Permitir caracteres duplicados",
            ...
            <div className="border-t border-slate-200 dark:border-slate-800 pt-6 space-y-4">
              <label htmlFor="entry-notes" className="flex flex-col gap-2">
                ...
                <textarea
                  ...
                  placeholder="Añadir detalles, códigos de respaldo o preguntas de seguridad..."
                  rows={4}
            ...
            <div className="mt-4 space-y-4">
              ...
              <p className="text-center text-xs text-slate-500 dark:text-slate-400">
                Estos datos se cifrarán con tu contraseña maestra antes de
                guardarse.
              </p>
            </div>
          </form>
